            ...
            return $contentStr;
				  ...
				  if (($time >= $ret['send_start_date']) or ($time <= $ret['send_end_date'])) {
                   ...
                   // $coupon = $user_bonus[$bonus_rand];
                       $contentStr = "\r\n关注送". $type_money . "元现金券:" . $new_bonus_sn . "\r\n过期时间:$use_end_date"."\r\n<a href='".$base_url."/user.php?act=bonus&send_bouns=$new_bonus_sn'>立即绑定</a>";
				    //$contentStr = count($user_bonus).'现金券'.$new_bonus_sn;
