...
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_img, g.market_price, g.team_price, g.shop_price AS org_price, '.
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$goods_list[$row['goods_id']]['team_price']    = price_format($row['team_price']);
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT user_name,headimgurl,is_subscribe,uname,pay_points, user_money, credit_line, last_login, is_validated FROM " .$GLOBALS['hhs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    $info['headimgurl']  =$row['headimgurl'] ;
	$info['uname']  =$row['uname'] ;
	$info['is_subscribe']  =$row['is_subscribe'] ;
