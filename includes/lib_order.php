...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0,$rec_type=0,$team_sign=0)
{
    ...
    clear_cart(5);
    /* 取得商品信息 */
    $sql = "SELECT g.is_miao,g.luck_num,g.luck_times, g.is_luck, g.suppliers_id,g.city_id,g.district_id,g.team_num,g.team_price,g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
    ...
    $_SESSION['luck_num'] = $goods['luck_num'];
    $_SESSION['is_miao'] = $goods['is_miao'];

    // 抽奖产品重新定义team_sign
    if($_SESSION['is_luck']){
       $_SESSION['team_sign'] = $GLOBALS['db']->getOne('select team_sign from '.$GLOBALS['hhs']->table('order_info').' where extension_id="'.$goods_id.'" and team_status = 1 and luck_times = "'.$_SESSION['luck_times'].'" order by order_id desc');
    }
    // end    
    //end funk
    ...
    $parent = array(
        ...
        'is_luck'       => $goods['is_luck'],
        'luck_times'       => $goods['luck_times'],
        'is_miao'       => $goods['is_miao'],
 ...
 * @return  array   优惠劵数组
 */
function user_bonus($user_id, $goods_amount = 0,$team_first=false, $type_id = false, $free_all = false)
{
    $day   = getdate();
    $today = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
    $where = '';
    ...
    $sql = "SELECT t.free_all, t.type_id, t.type_name, t.type_money, b.bonus_id,t.use_start_date, t.use_end_date  " .
        "FROM " . $GLOBALS['hhs']->table('bonus_type') . " AS t," .
        $GLOBALS['hhs']->table('user_bonus') . " AS b " .
        "WHERE t.type_id = b.bonus_type_id " .
        "AND t.use_start_date <= '$today' " .
...
//             {
//                 $GLOBALS['db']->query('update '.$GLOBALS['hhs']->table('goods').' set goods_number = team_num, luck_times = luck_times + 1 where goods_id = "'.$order_info['extension_id'].'"');
//                 //抽奖流程
//                 //所有参团的人
...
//                 $nums = count($rows)-1;
//                 //幸运数字
//                 $luck_num = $_SESSION['luck_num'];
//                 $lucker = array();
//                 while ( count($lucker) <= $luck_num) {
//                     $is_luck = rand(0,$nums);
//                     if(! in_array($is_luck, $lucker)){
//                         $lucker[] = $is_luck;
//                     }
//                 }
...
//                 if($r['shared_allow'])
//                 {
//                     $shared_money = $r['shared_money'];
//                     $change_desc = '分享购买提成';
                    
//                     $sql = "select u.`user_id`,u.`openid`,o.`order_id` FROM ".$GLOBALS['hhs']->table('order_info')." as o,".$GLOBALS['hhs']->table('users')." as u WHERE u.`user_id` = o.`shared_by` AND o.`team_sign` = " . $team_sign;
...
//                         }
//                         //余额调整
//                         log_account_change($row['user_id'], $shared_money, 0, 0, 0, $change_desc, ACT_OTHER);
//                         //设置为已调整
//                         $GLOBALS['db']->query('update '.$GLOBALS['hhs']->table('order_info').' set `is_sended` = 1 WHERE `order_id` = ' . $row['order_id']);
//                     }
//                 }
//                 //团长免单
//                 //目前只有微信可以退款
        ...
        if($team_num<=$rel_num){
            ...
            if($order_info['is_luck'] > 0)
            {
                ...
                foreach ($rows as $key => $row) {
                    ...
                    {
                        $title = '系统提示：';
                        $mark  = '如果未中奖，系统将安排微信红包退款。'; 
                        //更新lucker状态
                        $sql = "update ".$GLOBALS['hhs']->table('order_info')." set `is_lucker` = 0 WHERE `order_id` = " . $row['order_id'];
                        $GLOBALS['db']->query($sql);
                    }
                    //没中奖
                    else{
                        $title = '系统提示：'; //手气差了那么一点点，差点儿就中奖了！';
                        $mark  = '如果未中奖，系统将安排微信红包退款。';
                        // 微信红包退款
                        if(    $row['transaction_id']//微信支付
                            && $row['money_paid'] >= 1.00//大于1.00
