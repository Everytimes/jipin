...
if ($_REQUEST['act'] == 'list')
{
    ...
    for ($i = 0; $i < count($modules); $i++)
    {
        ...
        /* 检查该插件是否已经安装 */
        $sql = "SELECT shipping_id, shipping_code, shipping_name, shipping_desc, insure, support_cod,shipping_order FROM " .$hhs->table('shipping'). " WHERE shipping_code='" .$modules[$i]['code']. "' ORDER BY shipping_id";
        $row = $db->GetRow($sql);
        if ($row)
        {
            /* 插件已经安装了，获得名称以及描述 */
