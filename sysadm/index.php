...
elseif($_REQUEST['act']=='create_index_html')
{

	include_once("../operation_page_index.php");
...
elseif ($_REQUEST['act'] == 'changeteamstatus')
{
    ...
    include_once(ROOT_PATH . 'includes/lib_transaction.php.php');
    ...
    $sql="select team_sign,pay_time from ".$hhs->table('order_info')." where extension_code='team_goods' and team_status in(1,3) and team_first=1 and pay_status=2 and pay_time< ".$old_pay_time." order by order_id desc LIMIT 20";
...
elseif ($_REQUEST['act'] == 'sendlatestnews')
{
    ...
    $sql = "SELECT openid,user_id FROM ".$hhs->table('users')." WHERE `is_subscribe` = 1 and last_login > " . $last_login . " AND user_id in(1 ,2,13 ,12) AND is_send < " . $today . " LIMIT 300";
