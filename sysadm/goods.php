...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
	...
	// 优惠券
    $bonus_list = $db->getAll("SELECT `type_id`,`type_name` from ".$hhs->table('bonus_type')." WHERE free_all=1");
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $limit_buy_bumber =  $is_luck? 1 : $_POST['limit_buy_bumber']; 
    $luck_num = $is_luck ? (intval($_POST['luck_num'])>0?intval($_POST['luck_num']):1 ): 0;
    ...
    $is_promote = $is_luck ? $is_luck: $is_miao;//empty($promote_price) ? 0 : 1;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $hhs->table('goods') . " (bonus_free_all,is_miao,luck_times,luck_num,is_luck,lab_qgby, lab_zpbz, lab_qtth, lab_jkbs, lab_hwzy,ts_a,ts_b,ts_c,express,spe_region,default_shipping_id,default_shipping_fee,default_step_fee,spe_shipping_fee,spe_step_fee,bonus_allowed,discount_type,discount_amount,subscribe,district_id,city_id,limit_buy_one,limit_buy_bumber,is_check,is_nearby,sales_num,little_img,team_num,team_price,goods_name, goods_name_style, goods_sn, " .
