...

{
    ...
    $sql = "select SUM(`money_paid`) AS total_fee from " . $hhs->table('order_info') . " where suppliers_id ='$suppliers_id' AND " 
        . $ex_where;
    ...
    $sql = "select order_id,order_sn,pay_time,add_time,(`money_paid`) AS total_fee from " . $hhs->table('order_info') . " where ";
...

    {
        ...
        $goods_amount = $db->getOne('select SUM(`money_paid`) from '. $hhs->table('order_info') .' where suppliers_id ='.$suppliers_id . ' and '. $ex_where);
...

{
    ...
    if(empty($log)){
        ...
        foreach ($suppliers as $v) {
            ...
            $sql = "select SUM(`money_paid`) AS total_fee from " . $hhs->table('order_info') . " where suppliers_id ='$suppliers_id' " 
                . $ex_where;
            $total_all = floatval($db->getOne($sql));
            ...
            //check end
            //do the next
            $sql = "select order_id,order_sn,pay_time,add_time,(`money_paid`) AS total_fee from " . $hhs->table('order_info') . " where suppliers_id ='$suppliers_id' ";
            ...
            if ($order_list) {
                ...
                // 插入明细表
                // change the amount
                $goods_amount = $db->getOne('select SUM(`money_paid`) from '. $hhs->table('order_info') .' where suppliers_id ='.$suppliers_id . $ex_where);
...

{
    ...
    if($rows['user_id'] && in_array($is_check, array(1,-1))){
        ...
        $shareurl = $_SERVER['SERVER_NAME'].'/business/suppliers.php';
