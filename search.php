...
/*------------------------------------------------------ */
    ...
    /* 查询商品 */
    $sql = "SELECT g.goods_id, g.suppliers_id, g.goods_name, g.goods_number, g.market_price, g.is_new, g.is_best, g.is_hot, g.team_num,g.team_price, g.shop_price AS org_price, ".
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		if($row['suppliers_id'])
		{
		$sql="select suppliers_name,supp_logo from ".$GLOBALS['hhs']->table('suppliers')." where suppliers_id=".$row['suppliers_id'];
		$suppliers = $GLOBALS['db']->getRow($sql);
		$arr[$row['goods_id']]['suppliers_name'] = 	$suppliers['suppliers_name'];
		$arr[$row['goods_id']]['logo'] = 	$suppliers['supp_logo'];
		}
    }
