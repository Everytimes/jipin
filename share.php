...
if ($act == 'default'){
    ...
    $team_sign = isset($_REQUEST['team_sign']) ? intval($_REQUEST['team_sign']) : 0;//team_sign表示团的团号
    $shared_by = isset($_REQUEST['shared_by']) ? intval($_REQUEST['shared_by']) : 0;
    ...
    $is_team=0;
    $sql=" SELECT * FROM ".$hhs->table('order_info')." where team_sign=".$team_sign." and team_first=1 " ;//team_first表示什么意思
    ...
    $team_info['shared_by'] = $shared_by;
    $smarty->assign('team_info', $team_info);

    if($team_info['team_status']!=1){//$team_info['team_status']显示为1 的时候，表示组团已成功；为0表示组团还未满；
        $is_team=0;
    }
    /*
    ...
    */
    ...
    $sql=" SELECT count(*) FROM ".$hhs->table('order_info')." where team_sign=".$team_sign." and team_status>0" ;//获取当前团的订单信息；
    $count=$db->getOne($sql);
    //还差多少人
    ...
    $sql=" SELECT goods_brief FROM ".$hhs->table('goods')." where goods_id=".$goods_list[0]['goods_id'];
    $goods_describe=$db->getRow($sql);
   ...
   //'http://'.$_SERVER['HTTP_HOST']."/".$goods_list[0]['goods_thumb']
    ...
    if($goods_describe['goods_brief'])
    {
        $smarty->assign('desc', mb_substr($goods_describe['goods_brief'], 0,30,'utf-8')  );
    }
    else
    {
    ...
    //再加入user_id
    $link="http://" . $_SERVER['HTTP_HOST'] . "/share.php?team_sign=".$team_info['team_sign'] . '&shared_by=' . ($shared_by?$user_id:0);
    //end
