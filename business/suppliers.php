...
elseif($action =='order_operation')
{
    ...
    /* 订单号,城市,业务,完成时间,供应商,车型 ,业务,订单金额,售价,成本,供应商结算金额  */
    echo hhs_iconv(EC_CHARSET, 'GB2312', '序号') . "\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','订单ID')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','订单价格')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','团购ID')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','订单号')."\t";
//    echo hhs_iconv(EC_CHARSET,'GB2312','支付时间')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','商品编号')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','商品名称')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','购买数量')."\t";
//    echo hhs_iconv(EC_CHARSET,'GB2312','商品属性')."\t";
//    echo hhs_iconv(EC_CHARSET,'GB2312','团购开始时间')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','省')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','市')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','区县')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','地址电')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312',"电话")."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','收货人姓名')."\t";
//    echo hhs_iconv(EC_CHARSET,'GB2312','下单时间')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','快递编号')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','快递名称')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','快递单号')."\t";
    echo hhs_iconv(EC_CHARSET,'GB2312','支付金额')."\t";
    echo "\n";


    /*
     *  2016/05/13 w注释，以下内容为原来所有。
        echo hhs_iconv(EC_CHARSET, 'GB2312', $title) . "\t\n";
    //     订单号,城市,业务,完成时间,供应商,车型 ,业务,订单金额,售价,成本,供应商结算金额
        echo hhs_iconv(EC_CHARSET, 'GB2312', '序号') . "\t";
        echo hhs_iconv(EC_CHARSET, 'GB2312', '订单号 . "\t";
        ...
        echo hhs_iconv(EC_CHARSET, 'GB2312', '下单时间4') . "\t";
        echo hhs_iconv(EC_CHARSET, 'GB2312', '收货人') . "\t";
        echo hhs_iconv(EC_CHARSET, 'GB2312', '电话') . "\t";
        echo hhs_iconv(EC_CHARSET, 'GB2312', '地址') . "\t";
        echo hhs_iconv(EC_CHARSET, 'GB2312', '总金额') . "\t";
    ...
    {
        $sql="select * from ".$GLOBALS['hhs']->table('order_info')."where order_id=".$value['order_id']." limit 1";
        $result=$GLOBALS['db']->getAll($sql);
        $sql="select region_name from ".$hhs->table('region')." where region_id=".$result[0]['province'];
        $province=$db->getOne($sql);
        $sql="select region_name from ".$hhs->table('region')." where region_id=".$result[0]['city'];
        $city=$db->getOne($sql);
        $sql="select region_name from ".$hhs->table('region')." where region_id=".$result[0]['district'];
        $district=$db->getOne($sql);
//        获取商品信息；
        $sql="select  g.*,og.goods_number,og.goods_attr from ".$hhs->table('order_goods')." as og left join ".
            $hhs->table('goods')." as g on og.goods_id=g.goods_id where og.order_id=".$value['order_id']
        ;
        $goods=$db->getRow($sql);

        $value['short_order_time'] = local_date('Y-m-d',$value['add_time']);
        echo hhs_iconv(EC_CHARSET, 'GB2312',$key+1 ) . "\t";//序号
        echo hhs_iconv(EC_CHARSET, 'GB2312', $value['order_id']) . "\t";//订单id
        echo hhs_iconv(EC_CHARSET, 'GB2312', $value['total_fee']) . "\t";//订单价格
        echo hhs_iconv(EC_CHARSET, 'GB2312', $result[0]['team_sign']) . "\t";//团购id
        echo hhs_iconv(EC_CHARSET, 'GB2312', $value['order_sn']) . "\t";//订单 号
        echo hhs_iconv(EC_CHARSET, 'GB2312', $goods['goods_sn']) . "\t";//商品编号？
        echo hhs_iconv(EC_CHARSET, 'GB2312', $goods['goods_name']) . "\t";//商品名称？
        echo hhs_iconv(EC_CHARSET, 'GB2312', $goods['goods_number']) . "\t";//购买数量？
//        echo hhs_iconv(EC_CHARSET, 'GB2312', $result[0]['team_start_time']) . "\t";//团购开始时间
        echo hhs_iconv(EC_CHARSET, 'GB2312', $province) . "\t";//省
        echo hhs_iconv(EC_CHARSET, 'GB2312', $city) . "\t";//市
        echo hhs_iconv(EC_CHARSET, 'GB2312', $district) . "\t";//地区
        echo hhs_iconv(EC_CHARSET, 'GB2312', $result[0]['address']) . "\t";//地址
        echo hhs_iconv(EC_CHARSET, 'GB2312', $result[0]['mobile']) . "\t";//电话
        echo hhs_iconv(EC_CHARSET, 'GB2312', $result[0]['consignee']) . "\t";//收货人名字
        echo hhs_iconv(EC_CHARSET, 'GB2312', $result[0]['shipping_id']) . "\t";//快递编号
        echo hhs_iconv(EC_CHARSET, 'GB2312', $result[0]['shipping_name']) . "\t";//快递名称
        echo hhs_iconv(EC_CHARSET, 'GB2312', $result[0]['invoice_no']) . "\t";//快递单号
        echo hhs_iconv(EC_CHARSET, 'GB2312', $value['money_paid']) . "\t";//支付金额
         ...
         * 2016/05/13，w注释,以下内容为原来所有。
                ...
                echo hhs_iconv(EC_CHARSET, 'GB2312', $value['total_fee']) . "\t";
                echo hhs_iconv(EC_CHARSET, 'GB2312', $value['user_name']) . "\t";
                echo hhs_iconv(EC_CHARSET, 'GB2312', $value['team_first']) . "\t";
                echo hhs_iconv(EC_CHARSET, 'GB2312', $value['goods_num']) . "\t";
                echo hhs_iconv(EC_CHARSET, 'GB2312', $value['address']) . "\t";
                echo hhs_iconv(EC_CHARSET, 'GB2312', $value['mobile']) . "\t";
                echo hhs_iconv(EC_CHARSET, 'GB2312', $value['consignee']) . "\t";
                echo hhs_iconv(EC_CHARSET, 'GB2312', $value['short_order_time']) . "\t";//??
                echo hhs_iconv(EC_CHARSET, 'GB2312', $value['total_fee']) . "\t";//应付金额
                echo hhs_iconv(EC_CHARSET, 'GB2312', $value['shipping_status']) . "\t";//应付金额
                echo hhs_iconv(EC_CHARSET, 'GB2312', strip_tags($_LANG['os'][$value['order_status']].",".$_LANG['ps'][$value['pay_status']].",".$_LANG['ss'][$value['shipping_status']])) . "\t";
                echo hhs_iconv(EC_CHARSET, 'GB2312', $value['money_paid']) . "\t";*/
        echo "\n";
    }
    exit;
...
elseif($action =='drop_goods')
{
    ...
    $sql = "DELETE FROM " . $hhs->table('goods_activity') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $hhs->table('goods_express') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
