...
function category_get_goods($children, $brand, $min, $max, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.ts_a, g.ts_b, g.ts_c,g.goods_number,g.little_img,  g.sales_num, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['goods_id']]['ts_a']             = $row['ts_a'];
		$arr[$row['goods_id']]['ts_b']             = $row['ts_b'];
		$arr[$row['goods_id']]['ts_c']             = $row['ts_c'];
        ...
        $arr[$row['goods_id']]['like'] = $rec_id ? '' : 'un';
    }
	#print_r($arr);
    return $arr;
}
