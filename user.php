...
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer', 'refund', 'act_refund','comment','lottery_list');
...
elseif ($action == 'order_list')
{
    ...
    {
        $where = " and order_status in (0,1,5) and pay_status=2 and shipping_status=1 ";
    }
    //待收货
    if($_REQUEST['composite_status'] =='180')
    {
        $where = " and order_status in (0,1,5) and pay_status=2 and shipping_status=0 ";
    }
    //評論
...
elseif ($action == 'team_list')
{
    ...
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$hhs->table('order_info'). " WHERE user_id = '$user_id' and extension_code='team_goods' " . $where);
