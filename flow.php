...
if ($_REQUEST['step'] == 'add_to_cart') {
    ...
    //判断商品是否限购
    $limit_row = $db->getRow('select limit_buy_one,limit_buy_bumber,promote_end_date,is_luck,is_miao from ' . $hhs->table('goods') . ' where goods_id =' . $goods->goods_id . ' and  is_delete=0  ');
    ...
    if (($limit_row['is_luck'] || $limit_row['is_miao']) && gmtime() > $limit_row['promote_end_date']) {
        $result['error'] = 5;
    ...
    */
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        $sql = 'SELECT COUNT(*) FROM ' . $hhs->table('cart') . ' WHERE session_id = \'' . SESS_ID . '\' AND is_gift > 0';
        if ($db->getOne($sql) > 0) {
...
');
...
} elseif ($_REQUEST['step'] == 'get_address') {
    /*------------------------------------------------------ */
    //-- 改变配送方式
    /*------------------------------------------------------ */
    include_once 'includes/cls_json.php';
    ...
    $sql = $GLOBALS['db']->query('update ' . $GLOBALS['hhs']->table('users') . " set address_id='{$address_id}' where user_id=" . $_SESSION['user_id']);
    //改变当前用户的地址信息
    ...
    $consignee = array('address_id' => $address_list->address_id, 'consignee' => $address_list->consignee, 'country' => 1, 'province' => $address_list->province, 'city' => $address_list->city, 'district' => $address_list->district, 'area' => $address_list->area, 'email' => $address_list->email, 'address' => $address_list->address, 'zipcode' => $address_list->zipcode, 'tel' => $address_list->tel, 'mobile' => $address_list->mobile, 'best_time' => $address_list->best_time);
...
{
    $smarty->assign('bonus_ad', $_CFG['bonus_ad']);
    /*------------------------------------------------------ */
    //-- 订单确认
    /*------------------------------------------------------ */
    include_once 'includes/lib_transaction.php';
    ...
    /* 检查购物车中是否有商品 */
    $sql = 'SELECT COUNT(*) FROM ' . $hhs->table('cart') . ' WHERE session_id = \'' . SESS_ID . '\' ' . "AND parent_id = 0 AND is_gift = 0 AND rec_type = '{$flow_type}'";
    if ($db->getOne($sql) == 0) {
...
');
    ...
    if ($bonus_allowed && (!isset($_CFG['use_bonus']) || $_CFG['use_bonus'] == '1') && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS)) {
    ...
    // 免单券
    if ($bonus_free_all) {
        $user_bonus = user_bonus($_SESSION['user_id'], $total['goods_price'], $team_first, $bonus_free_all, 1);
    } else {
        foreach ($user_bonus as $key => $bonus) {
            if ($bonus['type_id'] == $bonus_free_all) {
...
', '', $_CFG['invoice_content']));
    ...
    /* 插入订单商品 */
    $sql = 'INSERT INTO ' . $hhs->table('order_goods') . '( ' . 'city_id,district_id,suppliers_id,order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, ' . 'goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id) ' . " SELECT city_id,district_id,suppliers_id,'{$new_order_id}', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, '{$_SESSION['extension_code']}' , parent_id, is_gift, goods_attr_id" . ' FROM ' . $hhs->table('cart') . ' WHERE session_id = \'' . SESS_ID . "' AND rec_type = '{$flow_type}'";
    $db->query($sql);
    /* 修改拍卖活动状态 */
    ...
    /* 如果订单金额为0 处理虚拟卡 */
    if ($order['order_amount'] <= 0) {
        $sql = 'SELECT goods_id, goods_name, goods_number AS num FROM ' . $GLOBALS['hhs']->table('cart') . ' WHERE is_real = 0 AND extension_code = \'virtual_card\'' . ' AND session_id = \'' . SESS_ID . "' AND rec_type = '{$flow_type}'";
        $res = $GLOBALS['db']->getAll($sql);
        $virtual_goods = array();
        foreach ($res as $row) {
        ...
        if ($virtual_goods and $flow_type != CART_GROUP_BUY_GOODS) {
            ...
            if (virtual_goods_ship($virtual_goods, $msg, $order['order_sn'], true)) {
                ...
                $sql = 'SELECT COUNT(*)' . ' FROM ' . $hhs->table('order_goods') . " WHERE order_id = '{$order['order_id']}' " . ' AND is_real = 1';
                if ($db->getOne($sql) <= 0) {
                    /* 修改订单状态 */
    ...
    $_POST['inv_content'] = isset($_POST['inv_content']) ? compile_str($_POST['inv_content']) : '';
    $_POST['postscript'] = isset($_POST['postscript']) ? compile_str($_POST['postscript']) : '';
    $order = array('shipping_id' => intval($_POST['shipping_id']), 'suppliers_id' => intval($_POST['suppliers_id']), 'point_id' => intval($_POST['point_id']), 'pay_id' => intval($_POST['payment']), 'pack_id' => isset($_POST['pack']) ? intval($_POST['pack']) : 0, 'card_id' => isset($_POST['card']) ? intval($_POST['card']) : 0, 'card_message' => trim($_POST['card_message']), 'surplus' => isset($_POST['surplus']) ? floatval($_POST['surplus']) : 0.0, 'integral' => isset($_POST['integral']) ? intval($_POST['integral']) : 0, 'bonus_id' => isset($_POST['bonus']) ? intval($_POST['bonus']) : 0, 'need_inv' => empty($_POST['need_inv']) ? 0 : 1, 'inv_type' => $_POST['inv_type'], 'inv_payee' => trim($_POST['inv_payee']), 'inv_content' => $_POST['inv_content'], 'postscript' => trim($_POST['postscript']), 'how_oos' => isset($_LANG['oos'][$_POST['how_oos']]) ? addslashes($_LANG['oos'][$_POST['how_oos']]) : '', 'need_insure' => isset($_POST['need_insure']) ? intval($_POST['need_insure']) : 0, 'user_id' => $_SESSION['user_id'], 'is_luck' => $_SESSION['is_luck'], 'luck_times' => $_SESSION['luck_times'], 'is_miao' => $_SESSION['is_miao'], 'add_time' => gmtime(), 'order_status' => OS_UNCONFIRMED, 'shipping_status' => SS_UNSHIPPED, 'pay_status' => PS_UNPAYED, 'agency_id' => get_agency_by_regions(array($consignee['country'], $consignee['province'], $consignee['city'], $consignee['district'])), 'lat' => trim($_POST['lat']), 'lng' => trim($_POST['lng']), 'city_id' => intval($_POST['city_id']), 'district_id' => intval($_POST['district_id']), 'checked_mobile' => trim($_POST['checked_mobile']), 'package_one' => isset($_POST['package_one']) ? intval($_POST['package_one']) : 0);
    /* 扩展信息 */
    if (isset($_SESSION['flow_type']) && intval($_SESSION['flow_type']) != CART_GENERAL_GOODS) {
        $order['extension_code'] = $_SESSION['extension_code'];
        $order['extension_id'] = $_SESSION['extension_id'];
...
');
    ...
    if ($favourable['act_type'] == FAT_GOODS) {
        ...
        /* 检查是否已在购物车 */
        $sql = 'SELECT goods_name' . ' FROM ' . $hhs->table('cart') . ' WHERE session_id = \'' . SESS_ID . '\'' . ' AND rec_type = \'' . CART_GENERAL_GOODS . '\'' . " AND is_gift = '{$act_id}'" . ' AND goods_id ' . db_create_in($_POST['gift']);
...
');
    ...
    if ($_SESSION['user_id'] > 0) {
        ...
        if (empty($count)) {
            $time = gmtime();
            $sql = 'INSERT INTO ' . $GLOBALS['hhs']->table('collect_goods') . ' (user_id, goods_id, add_time)' . "VALUES ('{$_SESSION['user_id']}', '{$goods_id}', '{$time}')";
            $db->query($sql);
        }
        flow_drop_cart_goods($rec_id);
    }
...
');
    ...
    /* 购物车中商品配件列表 */
    //取得购物车中基本件ID
    $sql = 'SELECT goods_id ' . 'FROM ' . $GLOBALS['hhs']->table('cart') . ' WHERE session_id = \'' . SESS_ID . '\' ' . 'AND rec_type = \'' . CART_GENERAL_GOODS . '\' ' . 'AND is_gift = 0 ' . 'AND extension_code <> \'package_buy\' ' . 'AND parent_id = 0 ';
...
{
    $sql = 'SELECT SUM(g.integral * c.goods_number) ' . 'FROM ' . $GLOBALS['hhs']->table('cart') . ' AS c, ' . $GLOBALS['hhs']->table('goods') . ' AS g ' . 'WHERE c.session_id = \'' . SESS_ID . '\' AND c.goods_id = g.goods_id AND c.is_gift = 0 AND g.integral > 0 ' . 'AND c.rec_type = \'' . CART_GENERAL_GOODS . '\'';
...
function flow_update_cart($arr)
{
    ...
    foreach ($arr as $key => $val) {
        ...
        //查询：
        $sql = 'SELECT `goods_id`, `goods_attr_id`, `product_id`, `extension_code` FROM' . $GLOBALS['hhs']->table('cart') . " WHERE rec_id='{$key}' AND session_id='" . SESS_ID . '\'';
        $goods = $GLOBALS['db']->getRow($sql);
        $sql = 'SELECT g.goods_name, g.goods_number ' . 'FROM ' . $GLOBALS['hhs']->table('goods') . ' AS g, ' . $GLOBALS['hhs']->table('cart') . ' AS c ' . "WHERE g.goods_id = c.goods_id AND c.rec_id = '{$key}'";
        ...
        if ($val > 0) {
            ...
            while ($offers_accessories_row = $GLOBALS['db']->fetchRow($offers_accessories_res)) {
                if ($row_num > $val) {
                    $sql = 'DELETE FROM ' . $GLOBALS['hhs']->table('cart') . ' WHERE session_id = \'' . SESS_ID . '\' ' . 'AND rec_id = \'' . $offers_accessories_row['rec_id'] . '\' LIMIT 1';
                    $GLOBALS['db']->query($sql);
                }
                $row_num++;
            }
            ...
            if ($goods['extension_code'] == 'package_buy') {
                //更新购物车中的商品数量
                $sql = 'UPDATE ' . $GLOBALS['hhs']->table('cart') . " SET goods_number = '{$val}' WHERE rec_id='{$key}' AND session_id='" . SESS_ID . '\'';
            } else {
                $attr_id = empty($goods['goods_attr_id']) ? array() : explode(',', $goods['goods_attr_id']);
                $goods_price = get_final_price($goods['goods_id'], $val, true, $attr_id);
                //更新购物车中的商品数量
                $sql = 'UPDATE ' . $GLOBALS['hhs']->table('cart') . " SET goods_number = '{$val}', goods_price = '{$goods_price}' WHERE rec_id='{$key}' AND session_id='" . SESS_ID . '\'';
            }
        } else {
            ...
            $sql = 'DELETE FROM ' . $GLOBALS['hhs']->table('cart') . " WHERE rec_id='{$key}' AND session_id='" . SESS_ID . '\'';
        }
        $GLOBALS['db']->query($sql);
    }
    /* 删除所有赠品 */
...
function flow_cart_stock($arr)
{
    foreach ($arr as $key => $val) {
        ...
        $sql = 'SELECT `goods_id`, `goods_attr_id`, `extension_code` FROM' . $GLOBALS['hhs']->table('cart') . " WHERE rec_id='{$key}' AND session_id='" . SESS_ID . '\'';
        $goods = $GLOBALS['db']->getRow($sql);
        $sql = 'SELECT g.goods_name, g.goods_number, c.product_id ' . 'FROM ' . $GLOBALS['hhs']->table('goods') . ' AS g, ' . $GLOBALS['hhs']->table('cart') . ' AS c ' . "WHERE g.goods_id = c.goods_id AND c.rec_id = '{$key}'";
        $row = $GLOBALS['db']->getRow($sql);
...
function flow_drop_cart_goods($id)
{
    ...
    if ($row) {
        ...
        if ($row['extension_code'] == 'package_buy') {
            $sql = 'DELETE FROM ' . $GLOBALS['hhs']->table('cart') . ' WHERE session_id = \'' . SESS_ID . '\' ' . "AND rec_id = '{$id}' LIMIT 1";
            ...
            $sql = 'DELETE FROM ' . $GLOBALS['hhs']->table('cart') . ' WHERE session_id = \'' . SESS_ID . '\' ' . "AND (rec_id IN ({$_del_str}) OR parent_id = '{$row['goods_id']}' OR is_gift <> 0)";
        } else {
            $sql = 'DELETE FROM ' . $GLOBALS['hhs']->table('cart') . ' WHERE session_id = \'' . SESS_ID . '\' ' . "AND rec_id = '{$id}' LIMIT 1";
        }
        $GLOBALS['db']->query($sql);
...
function favourable_list($user_rank)
{
    ...
    $user_rank = ',' . $user_rank . ',';
    $now = gmtime();
    $sql = 'SELECT * ' . 'FROM ' . $GLOBALS['hhs']->table('favourable_activity') . ' WHERE CONCAT(\',\', user_rank, \',\') LIKE \'%' . $user_rank . '%\'' . " AND start_time <= '{$now}' AND end_time >= '{$now}'" . ' AND act_type = \'' . FAT_GOODS . '\'' . ' ORDER BY sort_order';
    $res = $GLOBALS['db']->query($sql);
...
function act_range_desc($favourable)
{
    if ($favourable['act_range'] == FAR_BRAND) {
        $sql = 'SELECT brand_name FROM ' . $GLOBALS['hhs']->table('brand') . ' WHERE brand_id ' . db_create_in($favourable['act_range_ext']);
        return join(',', $GLOBALS['db']->getCol($sql));
    } elseif ($favourable['act_range'] == FAR_CATEGORY) {
        $sql = 'SELECT cat_name FROM ' . $GLOBALS['hhs']->table('category') . ' WHERE cat_id ' . db_create_in($favourable['act_range_ext']);
        return join(',', $GLOBALS['db']->getCol($sql));
    } elseif ($favourable['act_range'] == FAR_GOODS) {
        $sql = 'SELECT goods_name FROM ' . $GLOBALS['hhs']->table('goods') . ' WHERE goods_id ' . db_create_in($favourable['act_range_ext']);
        return join(',', $GLOBALS['db']->getCol($sql));
    } else {
        return '';
    }
...
{
    $list = array();
    $sql = 'SELECT is_gift, COUNT(*) AS num ' . 'FROM ' . $GLOBALS['hhs']->table('cart') . ' WHERE session_id = \'' . SESS_ID . '\'' . ' AND rec_type = \'' . CART_GENERAL_GOODS . '\'' . ' AND is_gift > 0' . ' GROUP BY is_gift';
    $res = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->fetchRow($res)) {
...
{
    $sql = 'INSERT INTO ' . $GLOBALS['hhs']->table('cart') . ' (' . 'user_id, session_id, goods_id, goods_sn, goods_name, market_price, goods_price, ' . 'goods_number, is_real, extension_code, parent_id, is_gift, rec_type ) ' . "SELECT '{$_SESSION['user_id']}', '" . SESS_ID . '\', goods_id, goods_sn, goods_name, market_price, ' . "'{$price}', 1, is_real, extension_code, 0, '{$act_id}', '" . CART_GENERAL_GOODS . '\' ' . 'FROM ' . $GLOBALS['hhs']->table('goods') . " WHERE goods_id = '{$id}'";
    $GLOBALS['db']->query($sql);
}
/**
 * 添加优惠活动（非赠品）到购物车
...
{
    $sql = 'INSERT INTO ' . $GLOBALS['hhs']->table('cart') . '(' . 'user_id, session_id, goods_id, goods_sn, goods_name, market_price, goods_price, ' . 'goods_number, is_real, extension_code, parent_id, is_gift, rec_type ) ' . "VALUES('{$_SESSION['user_id']}', '" . SESS_ID . "', 0, '', '{$act_name}', 0, " . '\'' . -1 * $amount . "', 1, 0, '', 0, '{$act_id}', '" . CART_GENERAL_GOODS . '\')';
    $GLOBALS['db']->query($sql);
}
/**
 * 取得购物车中某优惠活动范围内的总金额
...
{
    /* 查询优惠范围内商品总额的sql */
    $sql = 'SELECT SUM(c.goods_price * c.goods_number) ' . 'FROM ' . $GLOBALS['hhs']->table('cart') . ' AS c, ' . $GLOBALS['hhs']->table('goods') . ' AS g ' . 'WHERE c.goods_id = g.goods_id ' . 'AND c.session_id = \'' . SESS_ID . '\' ' . 'AND c.rec_type = \'' . CART_GENERAL_GOODS . '\' ' . 'AND c.is_gift = 0 ' . 'AND c.goods_id > 0 ';
    /* 根据优惠范围修正sql */
...
function get_shipping_point_list()
{
    ...
    $sql = 'SELECT a.*,rp.region_name as province,rc.region_name as city,rd.region_name as district ' . ' FROM ' . $GLOBALS['hhs']->table('shipping_point') . ' AS a left join ' . $GLOBALS['hhs']->table('region') . ' AS rp on a.province=rp.region_id left join ' . $GLOBALS['hhs']->table('region') . ' as rc on a.city=rc.region_id left join ' . $GLOBALS['hhs']->table('region') . ' as rd on a.district=rd.region_id ' . $andwhere;
