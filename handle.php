...
if($info['package_one'])
{
	if ($info['team_first'] > 1) {
		...
		//更新状态
		$sql = "UPDATE ".$hhs->table('order_info')." SET `order_status` = 1,`shipping_status` = 2,`pay_status` = 2 WHERE `team_sign` = " . $info['team_sign'];
		$db->query($sql);
	}
}
else{
	//更新状态
	$sql = "UPDATE ".$hhs->table('order_info')." SET `order_status` = 1,`shipping_status` = 2,`pay_status` = 2 WHERE `order_id` = " . $order_id;
	$db->query($sql);
