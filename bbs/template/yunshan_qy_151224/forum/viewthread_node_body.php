...
<!--{else}-->
	...
	<!--{if $_GET['from'] != 'preview' && !empty($post['ratelog'])}-->
		...
		<dl id="ratelog_$post[pid]" class="rate{if !empty($_G['cookie']['ratecollapse'])} rate_collapse{/if} unreset-box-sizing">
        <!--评分-->
			...
			<!--{/if}-->
				...
				<!--{if $_G['setting']['ratelogon']}-->
					...
					<p class="ratc unreset-box-sizing">
				...
				<!--{else}-->
					<ul class="cl unreset-box-sizing">
						<!--{loop $post['ratelog'] $uid $ratelog}-->
							<li>
