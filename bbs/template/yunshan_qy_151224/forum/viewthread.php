...
<!--{eval  $pidOfFirstAuthor =  C::t('forum_post')->fetch_pid_by_tid_authorid($_G[tid],$_G[forum_thread][authorid]);}-->
...
<div class="col-md-9">
  <div class="read-page-wrap cl">
    <div  class="read-page cl">
      <div id="pt" class="bm cl">
        <div class="z"> <a href="./" class="nvhm" title="{lang homepage}">$_G[setting][bbname]</a><em>&raquo;</em><a href="forum.php">{$_G[setting][navs][2][navname]}</a>$navigation <em>&rsaquo;</em> <a href="forum.php?mod=viewthread&tid=$_G[tid]">$_G[forum_thread][short_subject]</a> </div>
      </div>
      <div class="read-page-title">
      <!--lou-->
        ...
        <span class="xg1 ft12 ftnm hidden_ipad"> 
        ...
        <!--{/if}--> 
        <a href="forum.php?mod=viewthread&tid=$_G[tid]$fromuid" onclick="return copyThreadUrl(this, '$_G[setting][bbname]')" {if $fromuid}title="{lang share_url_copy_comment}"{/if}>[{lang share_url_copy}]</a> </span> 
        <!--{if !IS_ROBOT}-->
        <div class="y hidden_ipad"> 
      ...
      <!--//end post manage op 警告|屏蔽|删除|置顶-->
      <!--{/if}--> 
      
      <!--{if $modmenu['thread']}-->
      <div id="modmenu" class="xi2 pbm hidden_ipad"> 
        <!--{eval $modopt=0;}--> 
        <!--{if $_G['forum']['ismoderator']}--> 
   ...
   <!--{/if}-->
      ...
      <div id="postlist"> 
        ...
        <!--{if $_G['forum_thread']['replycredit'] > 0 }-->
            ...
            <img src="{IMGDIR}/thread_prize_s.png" class="vm" alt="{lang replycredit}" /><span class="pl_top_txt">{$_G['forum_thread']['replycredit']} {$_G['setting']['extcredits'][$_G[forum_thread][replycredit_rule][extcreditstype]][unit]}{$_G['setting']['extcredits'][$_G[forum_thread][replycredit_rule][extcreditstype]][title]}</span></div>
            <div class="col-xs-8 col-md-8">{lang thread_replycredit_tips1}{lang thread_replycredit_tips2}<!--{if $_G['forum_thread']['replycredit_rule'][random] > 0}--><span class="pl_top_txt">{lang thread_replycredit_tips3}</span><!--{/if}--></div></div>
            <!--//end 回帖奖励-->
            <!--{/if}--> 
            ...
            <div class="row">
                ...
                <img src="{IMGDIR}/rushreply_s.png" class="vm" alt="{lang rushreply}" /> <strong>{lang rushreply}</strong>
                <!--{if $rushresult[rewardfloor]}--> 
                <span class="y"> 
                <!--{if $_G['uid'] == $_G['thread']['authorid'] || $_G['forum']['ismoderator']}--><a href="javascript:;" onclick="showWindow('membernum', 'forum.php?mod=ajax&action=get_rushreply_membernum&tid=$_G[tid]')" class="y pl_top_pn"><span>{lang thread_rushreply_statnum}</span></a><!--{/if}--> 
                <!--{if !$_GET['checkrush']}--> 
                <a href="forum.php?mod=viewthread&tid=$post[tid]&checkrush=1" rel="nofollow" class="y pl_top_pn"><span>{lang rushreply_view}</span></a> 
                <!--{/if}--> 
                </span> 
                <!--{/if}-->
                </div>
        ...
        <!--{/if}-->
          <!--{if $post['first']}-->
           <!--{eval $_G['ys_forum_thread']['authortitle'] = $post['authortitle'];}-->
           <!--{if $post[stars]}--><!--{eval $_G['ys_forum_thread']['stars'] = showstars($post['stars']);}--><!--{/if}-->
           <!--{eval $_G['ys_forum_thread']['posts'] = $post['posts'];}-->
           <!--{eval $_G['ys_forum_thread']['threads'] = $post['threads'];}-->
           <!--{eval $_G['ys_forum_thread']['credits'] = $post['credits'];}-->
		  <!--{/if}--> 
        
        <div id="post_$post[pid]" class="{if $post['first']}floor-lou-wrap cl{else}floor-nolou-wrap cl{/if}" {if $_G['blockedpids'] && $post['inblacklist']}style="display:none;"{/if}> 
     ...
     <!--{if $multipage}-->
    <div class="yunshan_viewthread_pgt unreset-box-sizing cl">$multipage</div>
    <!--{/if}-->
    <!--{if $multipage && $page < $_G['setting']['threadmaxpages'] && $page < $_G['page_next']}--> 
  <!--{eval $nxtpage = $page + 1;}-->
  <div class="pgbtn unreset-box-sizing"><a href="forum.php?mod=viewthread&tid=$_G[tid]{if $_GET[authorid]}&authorid=$_GET[authorid]{/if}&page=$nxtpage" hidefocus="true" class="bm_h">{lang next_page_extra}</a></div>
  <!--{/if}-->
...
<div class="col-md-3">
<!--{if $_G['page'] == 1 }-->
  <div class="read-sidebar-user cl hidden-ipad">
    <div class="fsu-info cl">
      <div class="box-left fsu-left"> 
      <a href="home.php?mod=space&uid=$_G[forum_thread][authorid]" class="img-rounded"><!--{avatar($_G[forum_thread][authorid],small)}--></a>
      </div>
      <div class="box-right">
        <div class="fsu-right">
          <div class="name"><a href="home.php?mod=space&uid=$_G[forum_thread][authorid]" class="mr5">$_G[forum_thread][author]</a></div>
          <div class="level">
          <span class="stars">$_G['ys_forum_thread']['authortitle']</span></div>
        </div>
      </div>
    </div>
    <div class="fsu-operate ftyh cl">
    <a rel="nofollow" href="home.php?mod=spacecp&ac=friend&op=add&uid=$_G[forum_thread][authorid]&handlekey=addfriendhk_{$_G[forum_thread][authorid]}" id="a_friend_li_{$_G[forum_thread][authorid]}" onclick="showWindow(this.id, this.href, 'get', 0);" class="addfriend pull-left"><i class="fa fa-user-plus mr5"></i>加好友</a>
    <a href="home.php?mod=spacecp&ac=pm&op=showmsg&handlekey=showmsg_$_G[forum_thread][authorid]&touid=$_G[forum_thread][authorid]&pmid=0&daterange=2" id="a_sendpm_$_G[forum_thread][authorid]" onclick="showWindow('showMsgBox', this.href, 'get', 0)" class="ys_user_message pull-right"><i class="fa fa-envelope mr5"></i>写私信</a>
    <!--{if helper_access::check_module('follow')}-->
			<a href="home.php?mod=spacecp&ac=follow&op={if !empty($follow)}del{else}add{/if}&hash={FORMHASH}&fuid=$_G[forum_thread][authorid]" id="card_followmod_$_G[forum_thread][authorid]" onclick="showWindow(this.id, this.href, 'get', 0)" class="follow pull-left"><i class="fa fa-twitter mr5"></i><!--{if !empty($follow)}-->取消收听<!--{else}-->收听TA<!--{/if}--></a>
			<!--{/if}-->
	<a href="home.php?mod=spacecp&ac=poke&op=send&uid=$_G[forum_thread][authorid]&handlekey=propokehk_{$_G[forum_thread][authorid]}" id="a_poke_{$_G[forum_thread][authorid]}" onclick="showWindow(this.id, this.href, 'get', 0);" class="sayhi {if helper_access::check_module('follow') }pull-right{else}pull-left{/if}"><i class="fa fa-send mr5"></i>打招呼</a>
    </div>
    <div class="fsu-num cl">
      <ul>
      <li><a href="home.php?mod=space&uid=$_G[forum_thread][authorid]&do=thread&type=reply&view=me&from=space"><i>$_G['ys_forum_thread']['posts']</i><br>帖子</a></li>
	<li><a href="home.php?mod=space&uid=$_G[forum_thread][authorid]&do=thread&type=thread&view=me&from=space"><i>$_G['ys_forum_thread']['threads']</i><br>主题</a></li>
    <li><a href="home.php?mod=space&uid=$_G[forum_thread][authorid]&do=profile"><i>$_G['ys_forum_thread']['credits']</i><br>积分</a></li>
      </ul>
    </div>
  </div>
  <!--{/if}-->
  <div class="sidebar-box">
    <div class="box-title"><span class="name">推荐阅读</span> </div>
    <div class="read-a">
    <!--[diy=diyforumyunshanviewthreadrecommend]--><div id="diyforumyunshanviewthreadrecommend" class="area"></div><!--[/diy]-->
    </div>
  </div>
  <div class="sidebar-box">
    <div class="box-title"> <span class="name">热议话题</span> </div>
    <div class="read-b">
    <!--[diy=diyforumyunshanviewthreadhot]--><div id="diyforumyunshanviewthreadhot" class="area"></div><!--[/diy]-->
    </div>
  </div>
</div>
<!--//end col-md-3-->
...
<script type="text/javascript">
		jQuery('#modmenu span:last-child').css("visibility","hidden");
