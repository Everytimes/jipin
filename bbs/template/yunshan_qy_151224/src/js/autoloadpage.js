/*
	...
	modifi by yunshan http://addon.discuz.com/?@51353.developer for yunshan_qy_151224
*/
	...
	function getnextpagecontent() {
		...
		x.get(url, function (s) {
			...
			if(!picstyle) {
				...
				var nexts = s.match(/\<div id="normalthread_(\d+)" class="thread-list cl" \>(.+?)\<div class="myysline">\<\/div\>/g);
				//alert(nexts[0]);
				for(i in nexts) {
					if(i == 'index' || i == 'lastIndex') {
						continue;
					}
					var insertid = nexts[i].match(/<div id="normalthread_(\d+)" class="thread-list cl" \>/);
					//alert(insertid[1]);
