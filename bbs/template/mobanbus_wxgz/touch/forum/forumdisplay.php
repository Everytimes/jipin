...
<div class="bus_w100 bus_fl pt10 bus_forum_guid">
	<div class="tit">
	<!--{if $_G['forum'][icon]}-->
...
<!--{if ($_G['forum']['threadtypes'] && $_G['forum']['threadtypes']['listable']) || count($_G['forum']['threadsorts']['types']) > 0}-->
	...
	<ul id="thread_types" class="bus_sorta mt10">
...
<div class="clear"></div>
	...
	<!--{if $_G['forum_threadcount']}-->
		...
		<ul id="waterfall" class="mobanbus_item mobanbus_scroll">
			...
			<div class="user_bar">
				<span class="user_img"><!--{avatar($thread[authorid],normal)}--></span>
				<span class="user_info"><h5 class="name">$thread[author]</h5><h5 class="date">$thread[dateline]</h5></span>
			</div>
			<div class="clear"></div>
			<a class="newlist_bd" href="forum.php?mod=viewthread&tid=$thread[tid]&{if $_GET['archiveid']}archiveid={$_GET['archiveid']}&{/if}extra=$extra"$thread[highlight]{if $thread['isgroup'] == 1 || $thread['forumstick']} target="_blank"{else} onclick="atarget(this)"{/if}>
			<div class="bus_newslist_txt">
			<h2><!--{echo messagecutstr(DB::result_first('SELECT `message` FROM '.DB::table('forum_post').' WHERE `tid` ='.$thread[tid].' AND `first` =1'),120);}-->...</h2>
			</div>
			<img class="bus_newslist_pic" alt="{$thread[subject]}" src="{eval echo(getforumimg($thread['aid'],0,600,99999))}">
			</a>
			<div class="clear"></div>
			<div class="bus_newslist_info">
				<span class="bus_fl"><i class="icon-calendar pr10"></i><a href="forum.php?mod=forumdisplay&fid=$_G['forum'][fid]" title="$_G['forum']['name']" target="_self">$_G['forum']['name']</a></span><span class="bus_fl"><i class="icon-eye-open pr10"></i>{$thread[views]}</span><span class="bus_fl"><i class="icon-comments-alt pr10"></i>{$thread[replies]}</span>
			</div>
			</li>
			<!--{/loop}-->
		</ul>
